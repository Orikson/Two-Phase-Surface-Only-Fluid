...
class Camera {
    public:
         ...
         */
        Camera(glm::vec3 position = glm::vec3(0, 0, 0), glm::vec3 worldUp = glm::vec3(0, 0, 1), float yaw = YAW, float pitch = PITCH) : front(glm::vec3(0.0f, 0.0f, -1.0f)), movementSpeed(SPEED), mouseSens(SENSITIVITY), zoom(ZOOM), zoomSens(ZOOMSENSITIVITY) {
...
class ArcCamera : public Camera {
    public:
        ...
        // Default constructor
        ArcCamera() : ArcCamera(glm::vec3(0, 0, 0), 1) {}
         ...
         * @param yaw Yaw Euler orientation of camera. Defaults to -90
         * @param pitch Pitch Euler orientation of camera. Defaults to 0
         * @param minZ Minimum distance from center. Defaults to 1
         * @param maxZ Maximum distance from center. Defaults to 10
         */
        ArcCamera(glm::vec3 lookAt, float radius, float yaw = YAW, float pitch = PITCH, float minZ = 1.0f, float maxZ = 10.0f) : Camera() {
            this->center = lookAt;
            this->yaw = yaw;        // 0 -> 2PI
    ...
    private:
        glm::vec3 computePosition() {
            ...
            return glm::vec3(sin(p) * cos(y), sin(p) * sin(y), cos(p)) * radius + center;
        }
